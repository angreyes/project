 ...
 * @author wjforero
 */
public class ListMasculino implements ICommand {
...
// 1. Get the request parameters
        ...
        // 2. Do something with the data. usually you:
            ...
            List<Product> products = dao.findCategoria("5");
            // store the products in the request, so them can be painted in the view
            request.setAttribute("masculinoList", products);
            // 3. Finally, returns the command result
            result.setResult("listMasculino");
        } catch (Exception e) {
            Logger.getLogger(ListMasculino.class.getName()).log(Level.WARNING, null, e);
