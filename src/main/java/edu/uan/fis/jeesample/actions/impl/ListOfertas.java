 ...
 * @author wjforero
 */
public class ListOfertas implements ICommand {
...
// 1. Get the request parameters
        ...
        // 2. Do something with the data. usually you:
            ...
            List<Product> products = dao.findCategoria("6");
            // store the products in the request, so them can be painted in the view
            request.setAttribute("ofertasList", products);
            // 3. Finally, returns the command result
            result.setResult("listOfertas");
        } catch (Exception e) {
            Logger.getLogger(ListOfertas.class.getName()).log(Level.WARNING, null, e);
