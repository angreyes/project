 ...
 * @author wjforero
 */
public class ListPanties implements ICommand {
...
// 1. Get the request parameters
        ...
        // 2. Do something with the data. usually you:
            ...
            List<Product> products = dao.findCategoria("2");
            
            // store the products in the request, so them can be painted in the view
            request.setAttribute("pantiesList", products);
            // 3. Finally, returns the command result
            result.setResult("listPanties");
        } catch (Exception e) {
            Logger.getLogger(ListPanties.class.getName()).log(Level.WARNING, null, e);
