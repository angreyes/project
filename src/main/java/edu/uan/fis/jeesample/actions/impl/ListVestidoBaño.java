 ...
 * @author wjforero
 */
public class ListVestidoBaño implements ICommand {
...
// 1. Get the request parameters
        ...
        // 2. Do something with the data. usually you:
            ...
            List<Product> products = dao.findCategoria("4");
            // store the products in the request, so them can be painted in the view
            request.setAttribute("vestidoList", products);
            // 3. Finally, returns the command result
            result.setResult("listVestido");
        } catch (Exception e) {
            Logger.getLogger(ListVestidoBaño.class.getName()).log(Level.WARNING, null, e);
